...
export default {
    ...
    async execute(interaction){
        ...
        if (roles.includes(role.id)){
            const newRoles = roles.filter(r => r !== role.id).filter(e => !!e);
            db.set(`guild-${interaction.guildId}.clan_roles`, newRoles);

            return await interaction.reply({
                content: `Role "${role.name}" has been removed as clan role.`,
                flags: [MessageFlags.Ephemeral],
            });
        }

        return await interaction.reply({
            content: `Role "${role.name}" is not a clan role.`,
            flags: [MessageFlags.Ephemeral],
        });
    },
};
